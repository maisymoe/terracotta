...
export default new Subcommand({
    ...
    handler: (client, args, username, reply) => {
        ...
        if (!usePlayerPos && (args.length <= 3 || args.slice(0, 3).some(i => isNaN(Number(i))))) return reply("Invalid arguments! Needs either \"here\" or an X, Y and Z coordinate.");

        const player = client.bot.players[username];
        if (!player && usePlayerPos) return reply(`I can't see you...`);
        ...
        reply("Set pathfinder goal to given coordinate.");
    },
})
